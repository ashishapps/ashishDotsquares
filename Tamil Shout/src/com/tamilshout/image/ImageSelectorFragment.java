package com.tamilshout.image;

import java.io.File;

import android.app.AlertDialog;
import android.content.DialogInterface;
import android.content.Intent;
import android.net.Uri;
import android.provider.MediaStore;
import android.support.v4.app.Fragment;
import android.support.v4.app.FragmentActivity;

public class ImageSelectorFragment extends ImageSelector
{

	public static void openChooser(final Fragment act, final File file,
			final RemoveListener listener)
	{

		AlertDialog.Builder builder = new AlertDialog.Builder(act.getActivity());
		builder.setTitle("Select Picture");
		builder.setItems(DIALOG_OPTIONS, new DialogInterface.OnClickListener()
		{
			@Override
			public void onClick(DialogInterface dialog, int position)
			{

				if (position == 0)
					openGallary(act);
				else if (position == 1)
					openCamera(act, file);
				else if (position == 2 && listener != null)
					listener.onRemove();

			}
		});
		builder.create().show();
	}

	public static void openGallary(Fragment act)
	{

		Intent gallery = new Intent();
		gallery.setType("image/*");
		gallery.setAction(Intent.ACTION_GET_CONTENT);
		act.startActivityForResult(Intent.createChooser(gallery, "Select Picture"), IMAGE_GALLARY);

	}

	public static void openCamera(Fragment act, File file)
	{

		Intent camera = new Intent(
				android.provider.MediaStore.ACTION_IMAGE_CAPTURE);
		camera.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(file));
		act.startActivityForResult(camera, IMAGE_CAPTURE);
	}
	
	public static void openChooser(final FragmentActivity act, final File file,
			final RemoveListener listener)
	{
		
		AlertDialog.Builder builder = new AlertDialog.Builder(act);
		builder.setTitle("Select Picture");
		builder.setItems(DIALOG_OPTIONS, new DialogInterface.OnClickListener()
		{
			@Override
			public void onClick(DialogInterface dialog, int position)
			{
				
				if (position == 0)
					openGallary(act);
				else if (position == 1)
					openCamera(act, file);
				else if (position == 2 && listener != null)
					listener.onRemove();
				
			}
		});
		builder.create().show();
	}

}
